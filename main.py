...
TOKEN = "vk1.a.Z2D4u1sm2vQzeKF7SeOoSuBeyDKaYiPvKes7oUvHa3tKc8dOXhd6foneK6_22g_AzrAtTwmLfxtJlcGX1Hm6nMkdRPBht-1hVkd6XT7bQGZj7gWfxxoR5my-8tCtLhrebeLtX86wyYBBuBjVFT7ORVQafOQAt2CQRtlCIr2Z08isa5cx1D7rmBZ_4RhiUo0xzBe14dS5Vqt3aNhcX84ajA"
...
async def tech_handler(message: Message):
    ...
    await message.answer(
        "🚨 Техническая поддержка SimpleManager 🚨"
...
async def start_message(message: Message):
    return await message.answer("Приветствуем вас в боте SimpleManager!", keyboard=builder.reply([
        {
            "name": "Тех.Поддержка",
    ...
    for admin in admin_list:
        if admin.is_owner:
            if admin.member_id == message.from_id:
                ...
                return await message.answer("Активировать беседу может только создатель.")
    
    database['chats'][str(chat_id)]['is_active'] = True  # Активация чата
    DB.save(database)
    
    await message.answer("✅ Бот готов к работе! Создателю чата автоматически выдана роль \"Владелец\"")
...
@bot.on.message(IsCommand="roles")
async def roles_handler(message: Message):
    ...
    standart_roles = sorted(map(int, database[str(chat_id)]['standart_roles']), reverse = True)
    roles_list = []
    for role_priority in standart_roles:
        roles_list.append(f"-- {role_priority}: {database[str(chat_id)]['standart_roles'][str(role_priority)]}")
   
    await message.answer(f"🔰 Все роли данной беседы:\n\n" + ('\n').join(roles_list))
...
async def unmute_handler(message: Message):
    ...
    if str(user.id) not in database[str(chat_id)]["mutes"]:
        return await message.answer(f"⚠️ {get_nickname_id(user.id, 'У пользователя')} нет мута!")
...
async def help_message(message: Message):
    ...
    for cmd, priority in database[str(chat_id)]["command_priority"].items():
        command_list.append(
            f"-- /{cmd} [Приоритет: >{priority}]"
        )

    await message.answer(f"🔰 Все команды беседы:\n\n" + ("\n").join(command_list))
...
async def profile_message(message: Message):
    ...
    cursor = await resolveResources(mention) if mention != None else await get_user_info(message.from_id)

    if cursor == None:
        return await message.answer("❌ Неверный пользователь")

    if str(message.from_id) not in profiles:
        profiles[str(message.from_id)] = {
            "stats": {
                "balance": 0,
                "vip": 0
            }
    ...
    profile_str = f"🔖 Профиль пользователя {get_full_name(cursor)}\n\n"
    profile_str += f"💸 Рублей: {profiles[str(message.from_id)]['stats']['balance']}\n"
    profile_str += f"📏 VIP: {profiles[str(message.from_id)]['stats']['vip']}"
